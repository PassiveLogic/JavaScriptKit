...
public struct BridgeJSLink {
    ...
    private func generateVariableDeclarations() -> [String] {
        ...
        if hasGenerics {
            ...
            // initialization (setInstance runs earlier than that), so the
            // instantiator exposes an `afterInitialize` hook that hosts call
            // once the instance is initialized (the `instantiate.js` template
            // does). The guard below keeps the lazy path — first generic call
            // — as a fallback for instantiation paths that skip the hook.
    ...
    private func generateJavaScript(data: LinkData) throws -> String {
        ...
        // afterInitialize method: eager type-handle registration. Only emitted
        // when the generic runtime exists; otherwise the hook is absent and
        // callers use optional chaining.
        if hasGenerics {
            printer.indent {
                printer.write("afterInitialize: () => {")
                printer.indent {
                    printer.write("\(JSGlueVariableScope.reservedRegisterTypeHandles)();")
                }
                printer.write("},")
            }
        }
